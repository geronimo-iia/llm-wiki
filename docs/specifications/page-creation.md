...
`Mixture of Experts`). Type is inferred from the section prefix (`concepts/`
→ `concept`, `sources/` → `source-summary`, `queries/` → `query-result`,
anything else → `page`).
...
| Input | A `wiki://` URI | A source file/folder path + `--target` |
...
# LLM fills in frontmatter + body
wiki ingest /tmp/mixture-of-experts.md --target wiki://research/concepts
```
...
| `registry.rs` | Add `resolve_uri(uri) -> Result<(WikiEntry, slug)>` — shared with `wiki read` |
| `integrate.rs` | Add `create_page(slug, bundle, wiki_root)` and `create_section(slug, wiki_root)` |
| `markdown.rs` | Add `scaffold_frontmatter(slug)` — derive title and type from slug |
