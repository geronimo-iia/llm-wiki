---
title: "Configuration"
summary: "Two-level config — global registry at ~/.wiki/config.toml and per-wiki defaults at .wiki/config.toml. wiki config command for reading and writing both levels."
read_when:
...
- `~/.wiki/config.toml` — global: wiki registry, global defaults
- `.wiki/config.toml` — per-wiki: overrides and wiki-specific defaults
...
[index]
auto_rebuild = false  # true to rebuild stale index automatically before search/list/contradict
...
## 2. Per-Wiki Config — `.wiki/config.toml`

Lives inside the wiki git repository at `.wiki/config.toml`. Committed and
versioned with the wiki — shared across all users of the same wiki.
...
| `index.auto_rebuild` | global / per-wiki | `false` | Automatically rebuild stale index before search/list/contradict |
...
`wiki config set` without `--global` writes to the per-wiki config of the
...
| `registry.rs` | Load `[global]` and `[defaults]` alongside wiki registry; replace `default = true` on entries with `global.default_wiki` |
| `search.rs` | Read `search_top_k` from resolved config when `--top-k` not provided |
| `integrate.rs` | Read `page_mode` from resolved config when `--bundle` not provided |
| `server.rs` | Add `wiki_config` MCP tool |
...
| Feature | Status |
|---------|--------|
| `~/.wiki/config.toml` wiki registry | implemented |
| `.wiki/config.toml` per-wiki config | implemented (name only) |
