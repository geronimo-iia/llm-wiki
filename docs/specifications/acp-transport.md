...
→ tool_call: wiki_ingest({ path: "~/agent-skills/semantic-commit/", target: "wiki://research/skills" })
→ message: "Ingested 1 page + 2 assets. Reading for enrichment..."
→ tool_call: wiki_read({ uri: "wiki://research/skills/semantic-commit" })
→ message: "Producing enrichment..."
→ tool_call: wiki_ingest({ analysis: enrichment, target: "wiki://research" })
→ done: "+3 enrichments, 1 query result written"
```
...
lifetime. A session targets a specific wiki from the registry.
...
The LLM starts every session already knowing the enrichment contract and
workflow rules. No separate `wiki instruct` call needed.
...
```
prompt: "ingest ~/agent-skills/semantic-commit/ into research/skills"

wiki → tool_call: wiki_search({ query: "semantic commit", wiki: "research" })
wiki → tool_call: wiki_ingest({ path: "~/agent-skills/semantic-commit/", target: "wiki://research/skills" })
wiki → message: "Ingested: wiki://research/skills/semantic-commit + lifecycle.yaml co-located."
...
`WikiAgent` holds the full registry — all registered wikis are accessible
...
pub async fn serve_acp(registry: Arc<Registry>) -> Result<()> {
    let agent = WikiAgent::new(registry);
...
`acp.rs` calls the same functions as `server.rs` — `ingest::ingest`,
