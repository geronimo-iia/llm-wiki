---
title: "Crystallize"
summary: "Instruct workflow for distilling a chat session into a wiki page — the LLM writes a complete Markdown file and ingests it."
read_when:
...
writes a complete Markdown file (frontmatter + body) and ingests it via
`wiki ingest`. The workflow guides the LLM on *what to extract* from a
...
```
Sources  → wiki ingest (human or LLM writes the file)  → wiki pages
Chats    → crystallize workflow (LLM writes the file)   → wiki ingest → wiki pages
```
...
The LLM writes a complete Markdown file following the
[frontmatter authoring guide](frontmatter-authoring.md).
...
```
wiki_ingest(path, target: "wiki://research/queries")           # new page
wiki_ingest(path, target: "wiki://research/concepts", update: true)  # update
```
...
3. **Write a complete .md file** with frontmatter + body. Follow the
   `## frontmatter` guide. Use `type: query-result` for new crystallizations.
   Structure the body with: Summary, Decisions, Findings, Open Questions
   as appropriate.

4. **Ingest:** `wiki_ingest(path, target)` or
   `wiki_ingest(path, target, update: true)` for updates.
...
- The LLM already writes complete Markdown files for ingest
...
- `wiki ingest` already handles both create and update

Crystallize is a workflow, not a tool. The tool is `wiki ingest`.
