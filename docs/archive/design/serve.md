...
All active transports share the same wiki engine and registry. A request on
...
`--wiki` flag — the server is a registry-wide service.
...
```
1. Load ~/.wiki/config.toml — registry + global config
...
| `registry.rs` | Expose `load_all() -> Vec<WikiEntry>` for server startup |
