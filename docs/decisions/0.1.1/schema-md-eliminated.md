...
> The "type registry in `wiki.toml`" part of this decision has been
> superseded by [schema-driven-types](schema-driven-types.md) — types
> are now discovered from `schemas/*.json` via `x-wiki-types`, with
